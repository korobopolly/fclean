...
- **멀티스레드 스캔**: 네이티브 Linux 파일시스템에서 병렬 스캔으로 속도 향상 (WSL 9P 자동 감지)
...
| `--workers` | `-w` | 병렬 스캔 스레드 수 (0=자동, 기본값: 0) | `4` |
...
A: 네이티브 Linux 파일시스템(ext4 등)에서는 `--workers 4` 옵션으로 병렬 스캔을 활성화하면 속도가 향상됩니다. WSL `/mnt/c/` 경로는 9P 프로토콜의 제약으로 멀티스레드가 오히려 느리므로, 자동 감지를 통해 단일 스레드로 동작합니다. `--limit` 옵션으로 결과 표시 수를 제한하거나, `--pattern` 옵션으로 범위를 좁히는 것도 도움이 됩니다.
